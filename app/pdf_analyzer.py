...
class PDFAnalyzer:
    ...
    def pd_analyser(self, pdf_path: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if toc:
                ...
                title = doc.metadata.get('title') if doc.metadata.get('title') else self.txt_title_extract(doc)
                # Set title page to 0 for metadata titles, otherwise it's set in txt_title_extract
                ...
                return {"title": title, "outline": self.post_proc_hd_(outline)}
            ...
            text_blocks = self.txt_block_extractor(doc)
            if not text_blocks:
            ...
            headers, footers = self.hd_ft_search(text_blocks, len(doc))
            ...
            special_prefix = self.spc_title_check(text_blocks)
            ...
            font_stats = self.analyse_fonts(filtered_blocks)
            title = self.tle_ext(filtered_blocks, font_stats)
            ...
            toc_headings = self.toc_parser(filtered_blocks)
            if toc_headings:
                self.logger.info("Found and parsed a visual Table of Contents.")
                headings = self.post_proc_hd_(toc_headings)
            else:
                ...
                headings = self.ext_hd_val(heading_candidates, font_stats, text_blocks)
    ...
    def txt_title_extract(self, doc: fitz.Document) -> str:
        ...
        font_stats = self.analyse_fonts(text_blocks)
        return self.tle_ext(text_blocks, font_stats)

    def txt_block_extractor(self, doc: fitz.Document) -> List[Dict]:
                    ...
                    if block_lines:
                        ...
                        for i in range(1, len(block_lines)):
                            ...
                            if size_similar and font_similar and color_similar and y_close:
                                ...
                                if current_group:
                                    self.txt_add_block(current_group, text_blocks)
    ...
    def txt_add_block(self, line_group: List[Dict], text_blocks: List[Dict]):
    ...
    def analyse_fonts(self, text_blocks: List[Dict]) -> Dict:
    ...
    def tle_ext(self, text_blocks: List[Dict], font_stats: Dict) -> str:
        ...
        for block in first_page_blocks:
            text = block["text"].strip()
            if len(text) > 5 and not self.mtdata_like(text):
    ...
    def dt_rfp(self, first_page_blocks: List[Dict]) -> Optional[str]:
            ...
            if re.match(r'^RFP:\s*', text, re.IGNORECASE):
                ...
                for j in range(i + 1, min(i + 10, len(first_page_blocks))):  # Look ahead up to 10 blocks
                    ...
                    if (len(next_text) > 3 and 
                        not self.mtdata_like(next_text) and
    ...
    def multi_title_recon(self, main_block: Dict, all_blocks: List[Dict], block_index: int) -> str:
    ...
    def chk_overview(self, text_blocks: List[Dict]) -> Optional[str]:
    ...
    def seman_sc(self, block: Dict, font_stats: Dict, all_blocks: List[Dict], position: int) -> float:
        ...
        word_frequency = self.word_freq_calc(all_blocks)
    ...
    def word_freq_calc(self, blocks: List[Dict]) -> Dict[str, int]:
    ...
    def mtdata_like(self, text: str) -> bool:
    ...
    def spc_title_check(self, text_blocks: List[Dict]) -> Optional[str]:
    ...
    def hd_ft_search(self, text_blocks: List[Dict], total_pages: int) -> Tuple[List[str], List[str]]:
    ...
    def toc_parser(self, text_blocks: List[Dict]) -> List[Dict]:
                    ...
                    if len(match.groups()) == 3:
                        ...
                        try:
                            ...
                            toc_headings.append({
                                ...
                                "text": title.strip(),
                                "page": page_num,
                                # We'll get the y_pos in y_pos_adder
                            })
                        except ValueError:
                            continue
    ...
    def ext_hd_val(self, text_blocks: List[Dict], font_stats: Dict, all_blocks: List[Dict]) -> List[Dict]:
        ...
        toc_pages = self.id_toc_pg(page_blocks, max_page=5)  # Check first 5 pages
        ...
        for page_num, blocks in page_blocks.items():
            ...
            if page_num in toc_pages:
                ...
                toc_header = self.ext_toc_head(blocks, font_stats)
                if toc_header:
            ...
            for i, block in enumerate(blocks):
                ...
                if self.mostly_heading(block, font_stats):
                    # Use smart level detection that considers content patterns
                    level = self.hd_sm(block, font_stats)
                    
                    if level:
                        clean_text = self.hd_cleaner(text)
        ...
        return self.post_proc_hd_(headings)

    def id_toc_pg(self, page_blocks: Dict, max_page: int = 5) -> List[int]:
        """
    ...
    def ext_toc_head(self, blocks: List[Dict], font_stats: Dict) -> Optional[Dict]:
        """
    ...
    def dt_hd_lvl(self, font_size: float, h1_thresh: float, 
    ...
    def hd_sm(self, block: Dict, font_stats: Dict) -> Optional[str]:
        """
    ...
    def y_pos_adder(self, headings: List[Dict]) -> None:
    ...
    def hd_cleaner(self, text: str) -> str:
    ...
    def dup_det(self, heading_text: str, title: str, heading_page: int, title_page: int) -> bool:
        """
    ...
    def post_proc_hd_(self, headings: List[Dict]) -> List[Dict]:
        """
        ...
        for heading in headings:
            ...
            if text_key not in seen_texts:
                ...
                if self.dup_det(heading["text"], self._extracted_title, heading["page"], title_page):
                    continue
                    
                # Additional filtering for obvious non-headings
                if not self.nhd(heading["text"]):
                    # Check if heading has content following it
                    if self.ctnt_bel(heading):
        ...
        normalized_headings = self.norm_hd(cleaned_headings)
        
        # Apply advanced validation as final step
        final_headings = self.adv_hd_val(normalized_headings)
    ...
    def norm_hd(self, headings: List[Dict]) -> List[Dict]:
        """
    ...
    def ctnt_bel(self, heading: Dict) -> bool:
            ...
            for block in text_dict["blocks"]:
                ...
                if heading_found and block_text and block_y:
                    ...
                    if heading_y_position is None or block_y > heading_y_position:
                        ...
                        if len(block_text) > 15 and not self.mtdata_like(block_text):
    ...
    def extr_txt_ft(self, text: str) -> List[float]:
    ...
    def adv_hd_val(self, headings: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            for heading in headings:
                ...
                try:
                    features = self.extr_txt_ft(text)
